...
describe('Notification System', () => {
  ...
  const taskDeletedNotifications1 = notificationsRes1.body.filter(
    (n: any) => n.eventId.type === 'task_deleted' && !n.sent
  );
  expect(taskDeletedNotifications1.length).toBe(1); // Changed from 0 to 1
  ...
  expect(taskDeletedNotifications2.length).toBe(1); // Changed from 0 to 1
  ...
  // Fix: Access event data through n.eventId.data
  const bulkNotifications = notificationsRes.body.filter(
    (n: any) => n.eventId.data.title.includes('Bulk Task')
  );
